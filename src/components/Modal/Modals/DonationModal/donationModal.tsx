...
export default function DonationModal({ isOpen, onClose, materials, campaignId, onDonationAdded, userId }: DonationModalProps) {
  ...
  const handleDonate = async () => {
    if (selectedMaterial && quantity > 0) {
      ...
      const response = await fetch('https://db-liferay.onrender.com/donations', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        await fetch(`https://db-liferay.onrender.com/users/${userId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        const campaignResponse = await fetch(`https://db-liferay.onrender.com/campaigns/${campaignId}`);
        ...
        await fetch(`https://db-liferay.onrender.com/campaigns/${campaignId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
