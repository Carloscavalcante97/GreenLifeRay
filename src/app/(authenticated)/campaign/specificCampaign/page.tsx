...
function SpecificCampaign() {
  ...
  useEffect(() => {
    ...
    if (id) {
      fetch(`https://db-liferay.onrender.com/campaigns/${id}`)
        .then((response) => response.json())
        .then((data) => setCampaign(data));

      fetch(`https://db-liferay.onrender.com/donations?campaignId=${id}`)
        .then((response) => response.json())
  ...
  const handleParticipate = async () => {
    if (campaign && userId) {
      ...
      const response = await fetch(`https://db-liferay.onrender.com/campaigns/${campaign.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        const userResponse = await fetch(`https://db-liferay.onrender.com/users/${userId}`);
        const userData = await userResponse.json();
        const updatedCampaigns = [...userData.campaigns, campaign.id];
        const updatedUser = { ...userData, campaigns: updatedCampaigns };

        const userUpdateResponse = await fetch(`https://db-liferay.onrender.com/users/${userId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
